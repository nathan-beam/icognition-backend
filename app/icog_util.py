...
def truncate_text(text: str, tokenizer: AutoTokenizer, summarizer: Summarizer, LANGUAGE="english") -> str:
    max_length = tokenizer.model_max_length
    tokens_ids = tokenizer(text, return_tensors="pt").input_ids
    num_tokens = len(tokens_ids[0])

    if (num_tokens > max_length):
        logging.info(
            f"Text is too long. Splitting into chunks of {max_length} tokens")
        ...
        logging.info(
            f"Average tokens per sentence: {avg_tokens_per_sentence}.")
        logging.info(f"Excess tokens: {excess_tokens}.")
        logging.info(
            f"Number of sentences to summarize: {num_sentences_to_summarize}")
